...
bool is_loopback_ip(const std::string& remote_ip) {
  return remote_ip == "127.0.0.1" || remote_ip == "::1" || remote_ip.rfind("::ffff:127.", 0) == 0 || remote_ip.empty();
}
...
bool WebSocketGateway::start() {
  ...
  server_.setOnClientMessageCallback([&](std::shared_ptr<ix::ConnectionState> connection_state,
