...
nlohmann::json ContourExtractionService::available_ocr_languages() const {
  return list_tesseract_ocr_languages();
}
...
nlohmann::json ContourExtractionService::recognize_candidate_text(
  ...
  auto result = recognize_tesseract_ocr_text(extracted, params);
  result["sourceResultId"] = image_result_id;
  result["candidateId"] = candidate.value("candidateId", std::string{"manual"});
  return result;
}
