...
| Debug backend exe | `scripts\prepare-debug.ps1 -SkipFrontend` | `backend\out\build\windows-msvc-vcpkg\Debug\ReactMUIOpenCV.exe` |
...
3. `npm run typecheck`로 프론트엔드 타입 검사를 실행합니다.
...
- C++ 백엔드가 React UI를 정적으로 제공하려면 `frontend/dist`가 있어야 합니다.
- `frontend/dist`가 없으면 `/api/health`는 동작하지만 `/`에는 UI 대신 "Build frontend/dist" 안내가 표시됩니다.

앱 모드 UI까지 확인하려면 먼저 다음 중 하나를 실행합니다.
...
| `debug: prepare backend` | vcpkg 준비 + CMake configure + Debug build |
