...
새 PC에 VS Code와 Git만 설치되어 있다면 저장소 루트에서 bootstrap 스크립트를 먼저 실행합니다.

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\bootstrap.ps1
```

이 스크립트는 기본적으로 winget을 사용해 Node.js LTS, CMake, Visual Studio 2022 Build Tools, WebView2 Runtime을 확인하거나 설치하고, WebView2 SDK, npm 패키지, vcpkg, Release 빌드까지 이어서 수행합니다.

이미 필수 도구를 직접 설치했다면 도구 설치 단계는 건너뛸 수 있습니다.

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\bootstrap.ps1 -SkipToolInstall
```

프로젝트 스크립트는 설치된 Visual Studio 중 CMake가 지원하는 최신 generator를 자동 선택합니다. Visual Studio 2022 Build Tools 이상의 C++ workload와 Windows SDK가 설치되어 있어야 합니다. `backend/CMakePresets.json`은 CMake Tools 등에서 수동으로 사용할 수 있는 Visual Studio 2022 preset을 제공합니다.
...
| --- | --- | --- |
| 새 Windows PC 전체 bootstrap | `scripts\bootstrap.ps1` | 도구 설치, SDK, frontend deps, vcpkg, Release build |
| Windows 개발 도구 | `scripts\install-windows-prereqs.ps1` | Node.js, CMake, VS 2022 Build Tools, WebView2 Runtime |
| WebView2 SDK | `scripts\setup-webview2-sdk.ps1` | `%USERPROFILE%\.nuget\packages\microsoft.web.webview2` |
| Backend configure | `scripts\configure-backend.ps1` | `backend\out\build\windows-msvc-vcpkg` |
| Backend build | `scripts\build-backend.ps1` | Debug 또는 Release backend/app exe |
...
| React production bundle | `npm run build` 또는 `scripts\build.ps1` | `frontend\dist` |
| Release backend/app exe | `scripts\build.ps1` 또는 호환용 `.\build.ps1` | `backend\out\build\windows-msvc-vcpkg\Release` |
...
| `project: build release` | `scripts\build.ps1` 실행 |
...
powershell -ExecutionPolicy Bypass -File .\scripts\configure-backend.ps1
powershell -ExecutionPolicy Bypass -File .\scripts\build-backend.ps1 -Configuration Debug
```
...
Release 빌드는 `scripts` 아래의 빌드 스크립트를 사용합니다.
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\build.ps1 -SkipFrontendInstall
powershell -ExecutionPolicy Bypass -File .\scripts\build.ps1 -SkipFrontendBuild
powershell -ExecutionPolicy Bypass -File .\scripts\build.ps1 -SkipBackendBuild
```

일반적으로 배포 전에는 건너뛰지 않는 전체 `scripts\build.ps1`을 사용합니다. 기존 자동화와 정책 호환을 위해 루트의 `.\build.ps1`도 같은 빌드 스크립트로 위임됩니다.
...
publish 스크립트는 기본적으로 `scripts\build.ps1`을 먼저 실행하고 다음 결과를 만듭니다.
...
현재 CMake는 기본적으로 다음 SDK 경로를 찾습니다. `scripts\setup-webview2-sdk.ps1`가 이 위치에 SDK를 내려받습니다.
...
- Release 배포 전 `scripts\build.ps1`, 호환용 `.\build.ps1`, `scripts\publish.ps1`이 성공합니다.

## 13. 흔한 문제

### CMake configure가 generator 오류로 실패

프로젝트 스크립트는 generator를 자동 선택합니다. 수동으로 `backend/CMakePresets.json`을 사용할 때만 preset generator와 설치된 Visual Studio 버전이 맞아야 합니다. MSVC x64와 Windows SDK가 설치되어 있어야 합니다.
...
백엔드가 `frontend/dist`를 찾지 못했을 수 있습니다. `npm run build` 또는 `scripts\build.ps1`을 실행한 뒤 다시 시작합니다.
