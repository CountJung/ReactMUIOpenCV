...
방화벽 대화상자가 표시되면 개인 네트워크에서만 허용하고, 공용 네트워크 허용은 선택하지 않는 것을 권장합니다.
...
다른 장치에서 접속되지 않으면 다음을 확인합니다.

1. Windows 보안에서 `방화벽 및 네트워크 보호`를 엽니다.
2. 현재 연결된 네트워크가 신뢰할 수 있는 개인 네트워크인지 확인합니다.
3. `앱이 방화벽을 통과하도록 허용`에서 `ReactMUIOpenCV.exe`가 개인 네트워크에 허용되어 있는지 확인합니다.
4. 공용 Wi-Fi에서는 LAN 모드를 끄거나 방화벽 허용을 해제합니다.

LAN 모드는 PIN 또는 토큰 인증을 요구하며, 원격 클라이언트는 기본적으로 read-only 권한으로 시작합니다.
...
- LAN 클라이언트가 접속하지 못하면 Windows 방화벽의 개인 네트워크 허용 여부와 같은 네트워크 연결 여부를 확인합니다.
