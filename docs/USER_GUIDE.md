...
ReactMUIOpenCV is a Windows-first local vision/media testbed. The app runs a local C++ OpenCV backend and serves the shared React UI at `http://127.0.0.1:18730`.
...
The published bundle includes the backend executable, required runtime DLLs, and the built React UI. Node.js, CMake, and vcpkg are not required for end users.
...
4. The browser opens `http://127.0.0.1:18730`.
