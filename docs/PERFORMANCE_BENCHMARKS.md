...
3. Charts 화면으로 이동한다.
4. Job Type을 `Performance benchmark`로 선택한다.
5. Image Result에서 측정할 이미지를 선택한다.
6. Iterations를 1에서 20 사이로 지정한다. 기본값은 5다.
7. Assign을 누르면 `performance-benchmark` job이 생성되고 완료 후 결과가 저장된다.

## 결과 확인

- Dashboard의 `Benchmarks` 카드에서 전체 샘플 수, 최신 fastest method, `forEach` 대비 포인터 루프 배율을 확인한다.
- Charts의 `Pixel Benchmarks` 카드에서 최신 샘플의 method별 MP/s 처리량을 비교한다.
