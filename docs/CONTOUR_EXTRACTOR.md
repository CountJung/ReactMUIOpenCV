...
6. `OCR Languages`에서 `eng`, `kor`, 또는 `eng+kor`에 해당하는 언어 조합을 선택하고 `Page Segmentation`을 고른다.
7. `OCR`을 누르면 선택 후보를 정면 보정한 이미지에서 Tesseract OCR을 실행하고 `OCR Result`에 인식 텍스트, confidence, 라인별 결과를 표시한다.
8. `Extract`를 누르면 선택 후보만 `perspectiveExtract` Image Result로 저장되고, `Extraction History`에 누적된다.
9. 하단 `Save Latest Extracted PNG` 또는 히스토리 행의 save 버튼을 누르면 정면뷰로 추출된 결과 이미지를 별도 PNG 파일로 저장한다.
10. 후보 목록은 `Clear`로 비울 수 있고, 저장된 추출 결과는 히스토리 행의 delete 버튼으로 삭제할 수 있다.
...
- OCR 결과는 선택 후보에 대한 즉시 분석 결과로 화면에 표시된다. 현재 구현은 `libtesseract`를 사용하며 `eng+kor`처럼 `+`로 연결된 언어 조합을 지원한다.

## Tesseract 언어 데이터

- 백엔드는 `models/tessdata`, `TESSDATA_PREFIX`, 그리고 vcpkg의 일반적인 tessdata 위치를 순서대로 찾는다.
- 영어 OCR에는 `eng.traineddata`, 한글 OCR에는 `kor.traineddata`가 필요하다.
- 두 파일이 모두 있으면 기본 OCR 언어 조합은 `eng+kor`로 초기화된다.
- 대용량 `.traineddata` 파일은 Git에서 제외되며, 로컬에서는 `models/tessdata` 아래에 직접 배치하면 된다.
...
- `GET /api/contours/ocr/languages`
- `POST /api/contours/preview`
- `POST /api/contours/ocr`
- `POST /api/contours/extract`

`preview`, `ocr`, `extract` 요청 본문은 같은 후보 형태를 사용한다.
