...
6. `OCR`을 누르면 선택 후보를 정면 보정한 이미지에서 OpenCV 기반 템플릿 OCR을 실행하고 `OCR Result`에 인식 텍스트, confidence, 라인별 결과를 표시한다.
7. `Extract`를 누르면 선택 후보만 `perspectiveExtract` Image Result로 저장되고, `Extraction History`에 누적된다.
8. 하단 `Save Latest Extracted PNG` 또는 히스토리 행의 save 버튼을 누르면 정면뷰로 추출된 결과 이미지를 별도 PNG 파일로 저장한다.
9. 후보 목록은 `Clear`로 비울 수 있고, 저장된 추출 결과는 히스토리 행의 delete 버튼으로 삭제할 수 있다.
...
- OCR 결과는 선택 후보에 대한 즉시 분석 결과로 화면에 표시된다. 현재 구현은 외부 Tesseract 의존성 없이 OpenCV threshold, connected components, glyph template matching을 사용하는 경량 OCR이므로 고대비 대문자/숫자 라벨에 가장 적합하다.
