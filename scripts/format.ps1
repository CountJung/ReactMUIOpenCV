...
if (-not $SkipFrontend) {
  Invoke-CheckedCommand -Command "npm" -Arguments @("run", "format") -WorkingDirectory $FrontendDir
}

if (-not $SkipBackend) {
  $clangFormat = Resolve-ClangFormat
  if (-not $clangFormat) {
    throw "clang-format was not found. Install LLVM or the VS Code C/C++ extension before formatting backend sources."
  }
