...
- `data/performance-benchmarks.json`: Backend-owned persisted OpenCV pixel-access benchmark samples. Created at runtime when Performance Lab runs a benchmark job and still visualized by Dashboard, Charts, and Data Grid.
- `data/settings.json`: Backend-owned persisted app preferences, logging policy, and OpenCV expert defaults. Created at runtime when Settings are first read or saved; currently stores theme mode, Contour Extractor detection defaults, Performance Lab benchmark iteration limits, and rolling-log retention/size settings.
...
- `models/tessdata/README.md`: Optional Tesseract OCR language-data guide. Large `.traineddata` files are ignored by Git; the backend discovers language files under `models/tessdata` or `TESSDATA_PREFIX`.
...
- `docs/CONTOUR_EXTRACTOR.md`: Korean guide for running Pack 9I contour candidate detection, interactive quadrilateral selection, selected-region OCR, perspective extraction, and pipeline reuse.
...
- `frontend/src/api/`: REST and WebSocket client code. `remoteApi.ts` owns Remote Access and Network Info calls; `settingsApi.ts` owns backend settings, theme mode, OpenCV expert defaults, benchmark defaults, and rolling-log settings; `imageApi.ts` owns Image Lab open/upload/process/save/result/calibration/DNN model discovery/catalog/download calls; `contourApi.ts` owns contour candidate detection, non-persistent perspective preview, Tesseract language discovery, selected-region OCR, and perspective extraction calls; `videoApi.ts` owns Video Lab open/upload/frame/extract/export/motion metrics/tracking calls; `performanceApi.ts` owns OpenCV pixel benchmark job/history/delete calls; `pipelineApi.ts` owns Phase 6 pipeline document, CRUD, execution, and execution result types including image/video/tracking nodes; `jobsApi.ts`, `logsApi.ts`, and `filesApi.ts` feed Dashboard, Charts, Logs, and Data Grid state.
...
- `frontend/src/features/`: Route-level pages for dashboard, remote access, image/video lab, contour extractor, performance lab, pipeline, charts, data grid, logs, and settings. Image Lab keeps reusable operation labels/default parameters in `frontend/src/features/image-lab/imageOperations.ts`, parameter controls in `frontend/src/features/image-lab/ImageOperationControls.tsx`, and an opened/uploaded source picker in `ImageLabPage.tsx`. Contour Extractor lives in `frontend/src/features/contour-extractor/ContourExtractorPage.tsx` and provides direct open/upload, fixed source overlay selection, click-to-preview perspective view, selectable Tesseract OCR language combinations, selected-region OCR display, extraction history controls, and PNG save actions for extracted front-view results. Performance Lab lives in `frontend/src/features/performance-lab/PerformanceLabPage.tsx` and provides direct image selection/open/upload, settings-backed benchmark execution, comparison, and history cleanup. Settings exposes theme, contour defaults, benchmark iteration limits, and rolling-log retention/size controls. Video Lab exposes opened/uploaded videos as a reusable library in `VideoLabPage.tsx`. Pipeline Flow keeps reusable node/operation choices in `frontend/src/features/pipeline-flow/pipelineOptions.ts`.
...
- `backend/vcpkg.json`: C++ dependency manifest, including optional OpenCV DNN module support for model-backed examples.
...
- `backend/src/server/ApiServer.*`: HTTP server composition, core route registration, static React file serving from a resolved source-tree or bundled `frontend/dist`, and thin request/response translation over backend services. Feature route groups can live in focused `ApiServer*Routes.cpp` implementation files when `ApiServer.cpp` grows too large.
- `backend/src/server/ApiServerVisionRoutes.cpp`: Focused route registration for contour perspective extraction, selected-region OCR, and OpenCV performance benchmark APIs. Route callbacks capture request guards by value because they outlive route registration.
...
- `backend/src/logging/LogStore.*`: spdlog rolling-file setup, retention cleanup, recent log store, and `log.appended` event publication. Defaults are 5 retention days and 10 MB per file, controlled by backend settings.
- `backend/src/storage/SettingsStore.*`: Backend-owned settings state, validation, and `data/settings.json` persistence for app preferences, rolling-log policy, and OpenCV expert defaults such as contour detection and benchmark iteration defaults.
...
- `backend/src/vision/ContourExtractionService.*`: Interactive contour extraction service. Reads Image Lab result previews, ranks quadrilateral contour candidates, exposes candidate metadata, delegates selected-region OCR to Tesseract utilities, and delegates selected perspective extraction to the Image Result processing pipeline.
- `backend/src/vision/TesseractOcr.*`: Tesseract OCR helper. Discovers `.traineddata` files from `models/tessdata`, `TESSDATA_PREFIX`, or vcpkg tessdata locations; validates selected language combinations such as `eng+kor`; preprocesses selected contour images; and shapes OCR text, line, word-count, confidence, and method metadata.
...
- Contour extraction UI/API flow: start at `frontend/src/features/contour-extractor/ContourExtractorPage.tsx`, `frontend/src/api/contourApi.ts`, `backend/src/vision/ContourExtractionService.*`, and the `perspectiveExtract` operation in `backend/src/image/ImageFilters.cpp`.
- Performance benchmark UI/API flow: start at `frontend/src/features/performance-lab/PerformanceLabPage.tsx`, `frontend/src/features/chart-showcase/ChartShowcasePage.tsx`, `frontend/src/api/performanceApi.ts`, `frontend/src/api/settingsApi.ts`, `backend/src/server/ApiServerVisionRoutes.cpp`, `backend/src/vision/PerformanceBenchmarkService.*`, and `backend/src/storage/PerformanceBenchmarkStore.*`.
