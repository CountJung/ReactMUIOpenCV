...
- `docs/CONTOUR_EXTRACTOR.md`: Korean guide for running Pack 9I contour candidate detection, interactive quadrilateral selection, perspective extraction, and pipeline reuse.
...
- `backend/src/server/ApiServerVisionRoutes.cpp`: Focused route registration for contour perspective extraction and OpenCV performance benchmark APIs. Route callbacks capture request guards by value because they outlive route registration.
...
- `backend/src/vision/ContourExtractionService.*`: Interactive contour extraction service. Reads Image Lab result previews, ranks quadrilateral contour candidates, exposes candidate metadata, and delegates selected perspective extraction to the Image Result processing pipeline.
